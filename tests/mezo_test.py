...
from mezo import average_of_mezo_updates, mezo_update_step, reconstruct_mezo_updates
...
def test_mezo_update_step_reproducible(model: nn.Module, inputs: torch.Tensor, seed: int):
    ...
    projected_grad = mezo_update_step(
        model,
        inputs,
        loss_function,
        epsilon=epsilon,
    ...
    same_projected_grad = mezo_update_step(
        model,
        inputs,
        loss_function,
        epsilon=epsilon,
    ...
    other_projected_grad = mezo_update_step(
        model,
        inputs,
        loss_function,
        epsilon=epsilon,
